  ...
  # diag(데이터) : 데이터를 넣으면 대각원소반환, 데이터를 직접 선언하면 데이터를 대각원소로 가지는 대각행렬 선언
  A = matrix(1:4,2,2) 
  diag(1:5)
  diag(A) # : 1 4
  ...
  # length=n : n등분을 한다.
  # rep(c, n) 함수 : c는 상수, n은 반복수, 똑같은 원소 수열 선언 seq(-1,1,0.01)
  x <- seq(-5,5,length=n) 
  rep(1,5)
  ...
  # ylab="title" : y축에 이름을 title로 작성한다. plot(x,y)
  plot(x,y,type="p")
  plot(x,y,type="l")
  plot(x,y,type="o")
  plot(x,y,type="h")
  ...
  # 범례
  legend("topright",
         legend=c("N(0,1) pdf","critical line","rejection region","z(0.05)"), col=c("gray","blue","green","red"),
         lty=c(1,1,1,NA),
         lwd=c(2,2,2,NA),
         pch=c(NA,NA,NA,19))
  # 제목
  ...
  for(i in 1:n){
    plot(x,fx,type="l",lwd=2.0,bty="n") 
    abline(fx[i]-x[i]*dx[i],dx[i],lwd=2.0,col="red") 
    Sys.sleep(0.5)
  }
